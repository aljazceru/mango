...
- [ ] 32-03-PLAN.md — 6 AppActions + actor handlers (sync pipeline with 50-file batching, cascaded removal, UniFFI)
- [ ] 32-04-PLAN.md — Desktop iced UI + notify watcher + PollWatcher fallback + 5-min Tokio interval
- [ ] 32-05-PLAN.md — iOS SwiftUI + UIDocumentPickerViewController + .minimalBookmark lifecycle + ScenePhase sync + iCloud placeholder skip
- [ ] 32-06-PLAN.md — Android Compose + OpenDocumentTree + takePersistableUriPermission + bulk DocumentsContract traversal + WorkManager 15-min + onResume sync
