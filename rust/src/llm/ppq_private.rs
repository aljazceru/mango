...
/// Stream PPQ private chat completions from already-built async-openai API messages.
///
...
/// reach the PPQ private transport unchanged. The prior path collapsed Array → plain
/// text and silently dropped image parts
...
pub async fn run_streaming_chat_completion_from_api_messages(
    ...
    let response =
        match send_private_request(&backend, "/chat/completions", &model, body, true).await {
...
async fn verify_hpke_key_endpoint(
    ...
    if &parsed_key != expected_hpke_key {
        return Err(LlmError::NetworkError {
            reason: "HPKE key endpoint returned a different public key than the attested key"
                .to_string(),
        });
    }
    Ok(())
...
fn extract_dns_sans(cert: &X509Certificate<'_>) -> Result<Vec<String>, LlmError> {
    ...
    if sans.is_empty() {
        return Err(LlmError::NetworkError {
            reason: "Enclave certificate does not contain DNS Subject Alternative Names"
                .to_string(),
        });
    }
    Ok(sans)
...
fn domain_matches_sans(sans: &[String], expected_domain: &str) -> bool {
    ...
    sans.iter().any(|san| {
        san == expected_domain
            || (san.starts_with("*.")
                && san.trim_start_matches("*.") == parent
                && expected_domain != parent)
    })
}
...
fn decode_base32_upper(input: &str) -> Result<Vec<u8>, LlmError> {
    ...
    for byte in input.bytes() {
        ...
        let Some(index) = alphabet.iter().position(|candidate| *candidate == byte) else {
            return Err(LlmError::NetworkError {
                reason: format!(
                    "Invalid base32 character in certificate SAN data: {}",
                    byte as char
                ),
            });
        };
...
fn llm_to_attestation_error(error: LlmError) -> AttestationError {
    match error {
        LlmError::NetworkError { reason } | LlmError::AuthError { reason } => {
